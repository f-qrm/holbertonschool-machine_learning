...
class NST:
    ...
    def generate_image(self, iterations=1000, step=None, lr=0.01, beta1=0.9,
                       beta2=0.99):
        """Generate the neural style transferred image via gradient descent.

        Starts from a copy of the content image and iteratively updates its
        pixels using the Adam optimizer to minimize the total NST cost
        (content cost + style cost), tracking the best result seen so far.

        Args:
            iterations (int): Number of gradient descent steps. Default 1000.
            step (int or None): If not None, print progress every `step`
                iterations (and on the final iteration). Default None.
            lr (float or int): Learning rate for Adam. Default 0.01.
            beta1 (float): Adam beta_1 parameter, in [0, 1]. Default 0.9.
            beta2 (float): Adam beta_2 parameter, in [0, 1]. Default 0.99.

        Returns:
            tuple: (generated_image, cost)
                generated_image (np.ndarray): Best image found, shape (h, w, 3)
                cost (float): Total cost associated with that best image.

        Raises:
            TypeError: If iterations, step, lr, beta1, or beta2 have the
                wrong type.
            ValueError: If iterations, step, lr, beta1, or beta2 are outside
                their valid ranges.
        """
        ...
        # Start the generated image as a trainable copy of the content image
        generated_image = tf.Variable(self.content_image)
        optimizer = tf.keras.optimizers.Adam(
            learning_rate=lr, beta_1=beta1, beta_2=beta2)

        # Track the best (lowest-cost) image seen during optimization,
        # since the cost is not guaranteed to decrease monotonically
        best_cost = float('inf')
        best_image = None
        ...
        for i in range(iterations + 1):
            ...
            # Print progress at the requested step interval and on the
            # final iteration
            ...
            # Skip the update on the last pass — we've already recorded
            # its cost above and don't need to step beyond it
            if i < iterations:
                optimizer.apply_gradients([(grads, generated_image)])
                # Keep pixel values valid after each Adam update
    ...
    def variational_cost(generated_image):
        """Calculate the variational cost (pixel smoothness) of an image.

        Penalizes large differences between neighboring pixels, encouraging
