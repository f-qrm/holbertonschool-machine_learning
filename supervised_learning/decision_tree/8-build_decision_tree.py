...
class Decision_Tree():
    ...
    def possible_thresholds(self, node, feature):
        """Retourne tous les seuils possibles pour une feature en prenant
    ...
    def Gini_split_criterion_one_feature(self, node, feature):
        """Calcule le meilleur seuil pour une feature donnee en minimisant
            le Gini moyen pondere des deux enfants pour bien separer
            les classes et reduire l`impurete"""
        ...
        is_above = values[:, np.newaxis] > thresholds[np.newaxis, :]

        Left_F = is_above[:, :, np.newaxis] & is_class[:, np.newaxis, :]

        left_counts = np.sum(Left_F, axis=0)
        left_totals = np.sum(Left_F, axis=(0, 2))
        left_proportions = left_counts / left_totals[:, np.newaxis]
        left_gini = 1 - np.sum(left_proportions**2, axis=1)

        Right_F = ~is_above[:, :, np.newaxis] & is_class[:, np.newaxis, :]

        right_counts = np.sum(Right_F, axis=0)
        right_totals = np.sum(Right_F, axis=(0, 2))
        right_proportions = right_counts / right_totals[:, np.newaxis]
        right_gini = 1 - np.sum(right_proportions**2, axis=1)

        n = np.sum(node.sub_population)
        gini_split = (left_totals * left_gini + right_totals * right_gini) / n
        best_index = np.argmin(gini_split)
        return thresholds[best_index], gini_split[best_index]

    def Gini_split_criterion(self, node):
        """Retourne la meilleure feature et le meilleur seuil parmi
            toutes les features en choisissant celui qui minimise
            le Gini moyen"""
