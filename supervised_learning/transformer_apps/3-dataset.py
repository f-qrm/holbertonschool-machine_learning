...
class Dataset:
    ...
    def __init__(self, batch_size, max_len):
        """Initialize the train/validation splits, the tokenizers
        trained on them, and the batched/prefetched pipelines.

        Args:
            batch_size (int): batch size for training/validation.
            max_len (int): maximum number of tokens allowed for a
                sentence (either language) to be kept in the dataset.
        """
        ...
        # On rejette les phrases trop longues (> max_len tokens) pour
        # limiter la quantite de padding dans chaque batch : sans ce
        # filtre, une seule phrase tres longue forcerait tout le batch
        # a etre pad a sa taille, gaspillant memoire et calcul.
        self.data_train = self.data_train.filter(
            lambda pt, en: tf.logical_and(
                tf.size(pt) <= max_len, tf.size(en) <= max_len))
        # cache() evite de refaire le tokenize/filter a chaque epoque,
        # puisque ce resultat ne change jamais d'une epoque a l'autre.
        self.data_train = self.data_train.cache()
        # shuffle() re-melange les exemples a chaque epoque pour que le
        # modele ne voie pas toujours les phrases dans le meme ordre,
        # ce qui l'aiderait a memoriser plutot qu'a generaliser.
        self.data_train = self.data_train.shuffle(buffer_size=20000)
        # padded_batch() complete chaque phrase du batch avec des zeros
        # jusqu'a la longueur de la plus longue phrase du batch, car les
        # phrases n'ont pas toutes le meme nombre de tokens.
        self.data_train = self.data_train.padded_batch(batch_size)
        # prefetch() prepare le batch suivant pendant que le GPU traite
        # le batch courant, pour eviter que le GPU n'attende les donnees.
