...
class Yolo:
    ...
    def load_images(folder_path):
        ...
        """
        # Récupérer tous les chemins de fichiers dans le dossier
        image_paths = glob.glob(folder_path + '/*')
        # Charger chaque image en BGR avec OpenCV
    ...
    def preprocess_images(self, images):
        ...
        Resizes each image to the model's input dimensions using cubic
        interpolation, then normalizes pixel values to [0, 1].
        ...
        Returns:
            tuple: (pimages, image_shapes)
                - pimages (numpy.ndarray): preprocessed images,
                  shape (len(images), input_h, input_w, 3)
                - image_shapes (numpy.ndarray): original [height, width]
                  of each image, shape (len(images), 2)
        """
        pimages = []
        images_shape = []
        ...
        for image in images:
            ...
            width = image.shape[1]
            # Sauvegarder la taille originale pour recalculer les coords après
            images_shape.append([height, width])
            # Redimensionner à la taille du modèle avec interpolation cubique
            resized = cv2.resize(
    ...
    def predict(self, folder_path):
        """Run the full YOLO detection pipeline on a folder of images.

        Loads images, preprocesses them, runs the model, filters boxes
        and applies NMS, then displays each result in a window.
        Pressing 's' on any image saves it to detections/.
        ...
        Returns:
            tuple: (predictions, image_paths)
                - predictions (list of tuple): one entry per image,
                  each tuple is (box_predictions, predicted_box_classes,
                  predicted_box_scores)
                - image_paths (list of str): file paths of the images
        """
        images, image_paths = self.load_images(folder_path)
        pimages, image_shapes = self.preprocess_images(images)
        # Lancer l'inférence sur toutes les images prétraitées en une fois
        outputs = self.model.predict(pimages)
        predictions = []
        for i in range(len(images)):
            # Extraire les sorties du modèle correspondant à l'image i
            image_outputs = [outputs[j][i] for j in range(len(outputs))]
            ...
            # Supprimer les boîtes redondantes par NMS
            nms = self.non_max_suppression(
                filtered_boxes, box_classes, box_scores)
            box_predictions, predicted_box_classes, predicted_box_scores = nms
            # Afficher le résultat avec les boîtes finales
